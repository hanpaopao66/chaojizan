...
4. **邀请制开放开发者注册**:`developer_signup` 缺省就是邀请制,管理后台「小程序治理 → 开发者注册」加手机号;
5. 审核能力验证过(队列不积压、中位时长可接受)再改成 `open`。
...
| `developer_signup` 开发者注册 | 邀请制 | 在「小程序治理 → 开发者注册」改:邀请制 / 开放 / 暂停(已有开发者照常登录) |

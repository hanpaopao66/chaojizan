...
- 上传物是 zip;应用 ≤ 10 MB、小游戏 ≤ 30 MB,单文件 ≤ 5 MB,文件数 ≤ 1000,解压总量 ≤ 90 MB(防 zip 炸弹);**原来还有一条「≤ 3 倍压缩量」,2026-09-16 去掉**:zip 对文本的压缩比本来就远不止 3 倍(重复度高的 JS / JSON 实测 200 倍以上),它拒的全是正常包,而真炸弹由绝对上限兜住 ——而且那三条在解压时按实际字节数再核一遍,声明造假也撑不破;
...
## 执行记录(2026-09-12,#320–#338 全部落地,未 push、未部署)

提交:`10691b4` 服务端 · `571a969` SDK 与官方小程序 · `1b38075` 用户端容器 v2 · `f234ac7` 开发者后台、审核后台、App Links ·
`d8fc41e` 开发者文档 · `4174ab8` 官网 `/developers` `/miniapps` `/m/<appid>`、透明中心小程序栏 ·
`e045af2` 安全审计与急停闸 · `09f5de4` #338 验证里修的下拉手势 · `5441438` 不变量 I4 I6 I7 I8 的守卫 ·
`ecbaa44` 全量回归撞上的营销用例 · 最后一个提交是本记录和上线、合规、审计三份文档。

D1–D12 全部按「推荐」执行,只有 D1 后来改了:2026-09-12 定为**不另买域名,用主站下专门的一级 `mp.chaojizan.cc`**(应用在 `<appid>.mp.chaojizan.cc`),代价见 [安全审计记录](MINIAPP-SECURITY-AUDIT.md)「放在 chaojizan.cc 下面的代价」。代码里是配置项 `MINI_APP_HOST_DOMAIN`,没配时开发环境走 API 直出、生产上托管应用打不开;配成主站域名本身或它的上级会被当成没配(不然主站整站 404)。

### 和上面写的不一样的地方

1. **托管不用 nginx `auth_request`,由 api 直出**:托管域名的请求 nginx 原样反代,`MiniHostMiddleware` 按 Host 头认 appid
   (严格正则)、改写到内部路径,文件从私有桶读、带按应用生成的 CSP。鉴权、隔离、返回头都在一个进程里,入口 HTML
   `no-cache` 就能做到「隔离立即 404」,也不存在 auth_request 那个缓存键(#335 第 15 项)。代价是托管流量过 Python 进程,
   文件有进程内小缓存;量上来再加 nginx 缓存,键必须含 `$host`(见 [上线手册](MINIAPP-ROLLOUT.md));
2. **对象存储没另开 `superz-miniapps` 桶**,放在现有私有桶的 `miniapps/<appid>/<版本>/` 前缀下:私有桶本来就只经 api 出,
   效果一样,少一套凭据和一条桶策略;
3. 开发者账号是 `users.role = developer`(这一列是字符串存的枚举,不用改数据库类型);
4. **申诉改判不走状态机的正常迁移**:状态机不允许「驳回 → 审核通过」「移除 → 下架」这类回头路,改判是唯一的例外,
   代码里显式绕过、单独记一条 `appeal_overturned`,e2e 核了改判后版本真的回到「审核通过」;
5. **§5.4 加了 ping / pong**(#335 审计时补的):网页版宿主拦不住跨域 iframe 里页面自己跳去别的站,iframe 每次 load 后
   发 ping(targetOrigin 是托管 origin),8 秒没回答就清空并报错。代价:托管的每个 HTML 页都要引 SDK;
6. SDK 2.0.0 在发布前改过一次(加 ping / pong;没 push、没部署,所以没升版本号);
7. 官方小程序把 SDK 源码打进了自己的包里,SDK 一改两个包和 SHA-256 都要重打重发(已在开发库重发,
   现在是记事本 `57711856…`、2048 `3acc472c…`,和 `scripts/build_miniapp.sh` 的产物一致);
8. 急停闸(#337)做了三个:托管小程序、小程序目录、昵称头像,加上原有的开发者注册;定位等敏感能力还没开放,闸等开放时一起加;
9. **下拉手势(#338 在模拟器上验出来的)**:原先的「浅拉照旧刷新」在手机上拉不出来 —— RefreshIndicator 要拉到视口高度的
   1/6(常见手机约 130)才上膛,比面板阈值 120 还深,一拉就是面板。改成浅拉(过 56)松手时由首页替它扣扳机,
   露头条文案分两段(「松手刷新 · 继续下拉打开小程序」→「松手打开小程序」),widget 测试守着。
   顺带:首页有四处 `setState(() => _future = _load())`,箭头把 Future 交给了 setState,debug 下断言失败、那一轮不重建
   (release 下断言被剥掉才碰巧能用),一并改掉。

### 验证(#338)

- 服务端单测 924 个通过;小程序的 9 个 e2e(`e2e_mini_apps` 与 `e2e_miniapp_{identity,hosting,storage,dev,review,catalog,security,switches}`)
  在开发库副本上逐个跑,退出码都是 0;
- **全量 `make test`(171 个 e2e)**在开发库副本上跑:第 112 个 `e2e_marketing` 红了,和这批改动无关 —— 用例建的复购批次
  预算只有 100,演示店的沉睡老客按 id 排队领,库里攒了 1449 个,月初头一轮就被前 100 个领光,用例自己造的新用户永远排不到。
  已修用例(`ecbaa44`,预算开到上限);修后它和后面 59 个逐个跑,退出码全是 0 —— 171 个全绿;
- `make analyze` 全绿:四端 analyze、Dart 单测(shared 262、用户端 128、商家端 155、骑手端 114)、三个后台 tsc、
  SDK 19 个测试、官方小程序 tsc 与测试、文档检查(SDK 74 项与参考页一致、验签示例跑过测试向量、15 页站内链接);
- 浏览器(`scripts/verify_miniapp_browser.mjs`,真 Chrome 无头):XSS 五处渲染、CSP 五种拦截与上报、
  开发者后台模拟器里记事本新建 → 自动保存 → 返回列表 → 云存储里有、2048 方向键打一局且存档进云存储;
- App 网页版:下拉抽屉 → 记事本、2048;导航逃逸的测试应用被停止显示,站内换页不误报;
- **安卓模拟器**(本机那台,Android 16,debug 包连开发库副本):下拉抽屉 → 目录 → 打开记事本 → 飞行模式下新建一条
  (页面提示离线、这一条标「未同步」)→ 关飞行模式自动同步、服务端云存储里有 → 打开 2048 全屏、手势打一局 → 系统返回键关闭 →
  浅拉刷新(服务端收到重新拉取)、深拉开面板、在面板列表上往上推收起 → 冷启动直达链接直接打开记事本;
- 安全扫描通过;#335 的 16 项和 §3 的八条不变量都有检查、都弄坏过一次确认会红,记录见 [MINIAPP-SECURITY-AUDIT.md](MINIAPP-SECURITY-AUDIT.md)。

### 对照「完成的标志」

1. M0 + M1:开发库上 App 下拉能打开记事本和 2048,SHA-256 与开源仓构建产物一致 ✓;**生产还没部署**;
2. M2:注册 → 认证 → 建应用 → 模拟器里验签 → 提交审核 → 上线,e2e 和浏览器检查都走通;「陌生开发者只看文档」**还没找人试**;
3. §3 八条不变量都有守卫,都弄红过 ✓;
4. #335 每一项都有验证记录 ✓;
5. 透明中心有小程序的审核统计和下架记录 ✓;
6. 全量 e2e(171 个)、单测、analyze、安全扫描全绿 ✓。

### 还欠的

1. **真机**一遍(模拟器代替不了手感;DEV-PROMPTS-31 欠的那次一起);
2. **生产**:部署后插两条自家条目的 seed 要先征得同意;`*.mp.chaojizan.cc` 解析、泛域名证书(DNS-01)、签名密钥与离线备份,
   见 [上线手册](MINIAPP-ROLLOUT.md);
3. **法务**:[合规清单](MINIAPP-COMPLIANCE.md) 里的 12 条,尤其备案、用户协议与隐私政策、2048 要不要版号;
4. 找一个没参与的人只看文档走一遍 M2;
5. 用户端目前只有安卓,iOS 容器没有做过;定位、扫码、剪贴板、手机号(M3)没开放。

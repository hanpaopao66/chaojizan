...
D1–D12 全部按「推荐」执行。托管域名(D1)还没定,代码里是配置项 `MINI_APP_HOST_DOMAIN`,没配时开发环境走 API 直出、生产上托管应用打不开。
...
2. **生产**:部署后插两条自家条目的 seed 要先征得同意;托管域名(D1)、ICP 备案、泛域名证书(DNS-01)、签名密钥与离线备份,

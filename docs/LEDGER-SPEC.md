...
| 字段 | 类型 | 说明 |
|---|---|---|
| `schema` | int | 当前 `2`。历史锚点各自冻结着自己那天的值,按各自的口径校验 |
...
0. **schema 2(2026-08-24 起)**:演示号段(`138000000xx` / `938000000xx`)
   的流水**不再进入账本**。

   在此之前演示订单的账和真实账记在同一条链上,于是"清演示数据"必然
   连带清账本、连带重启链 —— 2026-07-28 那次重启就是这么来的。
   口径分开之后,清演示数据不再需要动链。

   这是一次**不兼容的口径变更**(同一天重算会得到不同结果),所以升了
   版本号。schema 1 的历史锚点照旧按 schema 1 校验:它们存的是当天的
   payload 全文,不是实时查询,演示单据删掉之后仍然自洽。

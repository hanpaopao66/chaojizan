...
> **执行状态(2026-07-29):#101–#113 全部完成并逐条提交。**
...
| 113 | 缺图占位体系(SzImage/SzCover)+ 商家补图推力 | `packages/shared` + 三端 12 处 |
...
```
把三端"商家/菜品没传图就很难看"这件事系统解决。先读 docs/DEV-PROMPTS-8.md 的「设计基线」。

现状:三端有 12 处缺图兜底、12 个 CircleAvatar、15 个灰图标,每处各写各的——有的灰图标、有的名称首字、有的 emoji。问题不是"没有占位",是占位不成体系:首页列表十家店排下来一片灰方块,读起来像页面没加载完。
业务规则(已拍板):
- 新增 SzImage(方形/圆形)与 SzCover(宽幅封面)两个组件,**三端所有会缺图的地方都走它**;
- 占位三层:① 底色按名称哈希取自六组泥土色(与骨白纸底同色域,不是彩虹色板),同一家店永远同一个色,用户能靠颜色认店;② 中间是名称首字(中文一字、拉丁两字母,「运营测试店-1784465885」只取「运」不带编号);③ 大尺寸时右下角压一个极淡的品类符号做纹样;
- 品类符号用 Material 图标(已在包里,零额外体积),不用 emoji——emoji 在不同 Android 机型渲染差异很大,且与这套观感不搭。金刚区的 emoji 保持不动,那是另一个用途;
- 加载中显示同一个占位而不是转圈:列表里十个转圈比十个色块更难看;
- **编辑侧例外**:商家传店铺 logo、用户换头像这些位置,占位之外必须保留相机角标——那里要的是"提醒你补图",提示不能被好看的占位吃掉;
- 补图推力:菜品列表里缺图的行挂一个「缺图」chip(原先是混在一长串灰字里的「· 建议配图」,基本看不见),页顶加一条汇总(只在确实有缺图时出现,补完自动消失)。
技术要点:哈希用名称不用数据库 id,这样演示数据与生产数据观感一致;深浅两套色对分别给值;首字用 szFigure(衬线),与金额同一套字。
验收:三端 flutter analyze 零问题、release 包能出;首页列表十家缺图商家并排看有层次而不是一片灰;商家端菜品页缺图 chip 与汇总条正确出现/消失。
```
...
- ~~商家端与骑手端只做了令牌替换与核心屏的结构调整,住宿子目录与部分二级页
  信息层级没重排~~ → **已由 #112 做完**(2026-07-29)。
...
**第一批 · 信息层级重排**
- 商家对账页:顶部先回答「能提多少」,新增「今天」分组
  (菜品流水 − 平台佣金 = 今日实收 · N 单),按日账单与提现记录
  从一堆 Card+ListTile 收成两张 SzCard 分隔行。
- 骑手钱包页:顶部回答「能提多少 / 今天跑了多少 / 什么时候到账」,
  12 个散落 ListTile 收成「保障与规则」「账目与账户」两组。
- 住宿子目录 6 个文件全部与外卖侧对齐(待处理左侧 clay 边、
  状态 SzChip、实收 earn / 佣金 hold、空态 SzEmpty)。

**第二批 · 硬编码语义色清零**
- 三端 60 处 `Colors.green/orange/grey/red/blue` 按语义映射到令牌。
  这批不是"没做完",是 #101 只覆盖了旧的色值**常量**,
  散在页面里的 Material 调色板颜色是另一类残留——它们在深色下不跟随主题,
  而且同一个"提醒"在不同页面可能是 orange / deepOrange / amber 三种颜色。
- `share_card.dart` 例外并加注释:离屏渲染成图片,没有 context 且底永远是白的。

顺带修正的判断:
- 住宿日历的「关房」原先是红色 → 那是店家自己关的,不是错误,改弱化色;
  「余 0」才是要你动手的信号,那个才走 danger。
- 房型下架原先用删除线 → 长房型名上删除线很难读,改弱化色。
- 点评页「我的回复」原先靠换字色区分 → 改 surfaceAlt 引用块衬底,
  一眼分得清谁说的。

...
- 托管域名(D1 已定为 `mp.chaojizan.cc`,见下一节)、证书、nginx 都还没上生产,Host 头、frame-ancestors、主域名挡内部路径
  这几条是按生产配置跑的单测,上线后要在生产上照[上线文档](MINIAPP-ROLLOUT.md)的检查清单再验一遍。

## 放在 chaojizan.cc 下面的代价(D1,2026-09-12)

托管不另买域名,用 `<appid>.mp.chaojizan.cc`。chaojizan.cc 不在公共后缀列表里,浏览器眼里这些子域名和主站是
**同一个站点、不同源**:读不了主站的 localStorage 和页面,但能给 `.chaojizan.cc` 种 cookie,也被当成「自家」地址。

核过、不构成问题的:

- **登录态**:服务端、官网、管理后台、商家后台、开发者后台都不用 cookie(代码里没有一处 cookie 读写),
  登录态是 `Authorization: Bearer`,令牌在各自 origin 的 localStorage 里。子域名种 cookie 种不进任何登录态。
  **以后谁要改成 cookie 登录,先把托管挪出 chaojizan.cc**;
- **CORS**:主站接口按 origin 白名单放行(`CORS_ORIGINS`),不按后缀,托管 origin 不在里面;
- **客户端的信任判断**:App 容器、网页版宿主、开发者后台模拟器都按服务端给的 origin 精确比,
  没有「chaojizan.cc 结尾就信」的写法;`document.domain` 放宽要两边都设,主站从来不设;
- **配错**:`MINI_APP_HOST_DOMAIN` 配成 `chaojizan.cc`(或 `cc`)会被当成没配 —— 否则主站的每个请求都会被当成托管请求,
  整站 404。单测 `test_host_domain_cannot_swallow_the_main_site`,去掉这道防护 → 红;中间件绕过它直接读配置 → 红。

剩下的,代码消不掉,只能靠审核和处置兜:

1. **cookie 塞满**:小程序的脚本能给 `.chaojizan.cc` 种满 cookie(浏览器每个域名能存上百个、每个 4KB)。之后这个浏览器
   请求主站时请求头超长,nginx 回 400,主站网页在这个浏览器里打不开,直到 cookie 过期或被清掉。App 的接口请求不走网页容器、
   不带 cookie,不受影响;受影响的是 App 网页容器里打开的主站页面(透明中心、公开账本两个条目)和浏览器里的官网。
   偷不到东西,只能捣乱,而且只影响打开过这个小程序的人。没做的缓解:nginx 遇到请求头过大时回 `Clear-Site-Data: "cookies"`
   让浏览器自己清。
2. **看着像官方页面**:地址都是 chaojizan.cc 结尾。链接被单独拿出来(短信、群聊)在浏览器里打开时,用户容易当成官方页面,
   而页面内容是开发者写的,可以做成仿冒的登录页、活动页;浏览器存过的 chaojizan.cc 的密码,在这些子域名的登录框里
   也会列出来让人点选。App 里打开时顶部写着「由 某某 提供」(全屏模式在「更多」菜单里),浏览器里没有。版本不可变、换内容要重新审核,能挡一部分。
   没做的缓解:托管页被浏览器直接打开(不是在 App 或网页版宿主里)时先出一页「这是第三方小程序,不是超级赞官方页面」。
   合规清单第 2 条也问了法务。
3. **同一个站点可能同一个进程**:桌面 Chrome 按「站点」分进程,管理后台审核预览、网页版宿主里嵌的托管页和外面的主站页面
   可能在同一个进程里,理论上存在 Spectre 这类旁路读内存的攻击面(外面的页面内存里有登录令牌;审核预览时就是管理员的)。
   单独注册的域名是跨站,浏览器会放进不同进程。手机 App 不受影响:令牌在 Dart 里,不在网页容器里。

和单独买一个域名比,多出来的代价主要是这三条。要彻底消掉,只能换单独的注册域名(改 `MINI_APP_HOST_DOMAIN`、换证书即可,
代码不用动;小程序的直达链接是 `chaojizan.cc/m/<appid>`,不受影响)。

...
- **顶栏**是宿主原生画在网页外面的,页面盖不住。排法照 Telegram:左边关闭(页面显示了 BackButton 时换成返回箭头),
  中间图标和名称,名称下面一行「由 XX 提供」和认证标记,右边 `···`。「由 XX 提供」这一行 Telegram 没有,超级赞留着 ——
  它是用户分辨「这是谁做的」的依据。顶栏底色跟你的 `setHeaderColor` 走(没设就是 `header_bg_color`),字色宿主按底色深浅自动取。
...
- **主按钮 / 次按钮**在底栏,由宿主原生画,页面只给文字和状态(`MainButton`)。和 Telegram 一样是通栏按钮,
  底栏底色是 `bottom_bar_bg_color`(卡片色,`setBottomBarColor` 改得了);次按钮默认和底栏同底、字是按钮色;

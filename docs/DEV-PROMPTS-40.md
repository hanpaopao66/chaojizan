...
| 全局搜索、会话内搜索、共享媒体 / 文件 / 链接 / 语音页 | 同;**2026-09-16 全局搜索加了顶部那一排分类**(对齐 Telegram):全部 / 对话 / 频道 / 应用 / 多媒体 / 下载内容 / 链接 / 文件 / 音乐 / 语音。服务端是 `/chat/v1/search?tab=…`(SEARCH_TABS),跨会话翻页用消息自增 id;「应用」= 小程序 + 机器人(各自的上架 / 开关判据照旧,不另写一份);「音乐」= 分享进聊天的歌卡片 + mime 是 audio/* 的文件(我们有音乐模块,这一格比 Telegram 那个更实);**「下载内容」只在客户端本机**(chat/downloads.dart)—— 谁把哪个文件存进了自己手机,服务端不知道也不该知道,做成服务端的表等于凭空多一份「谁下载了什么」的档案 | M2 |

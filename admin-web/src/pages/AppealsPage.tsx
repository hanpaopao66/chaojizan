 ...
 *  2026-09-15 定:平台判错了,平台自己认 —— 三方改判的钱都由平台补回,确认框里照实写每一支补什么 */
function overturnEffect(a: Appeal): string {
  switch (a.target_type) {
    case 'after_sale':
      return '撤销商家责任,信用分那一条不再计分;这单冲掉的净额和另出的配送费、小费补回商家,'
        + '钱由平台出(平台判错了,平台自己认);顾客拿到的退款不追回'
    case 'after_sale_rider':
      return '撤销骑手责任,判责时扣骑手的钱退回、保障金池出的回池(平台判错了,平台自己认)'
    case 'delivery_issue':
      return a.role === 'customer'
        ? '撤销「顾客原因」,顾客付的餐费由平台原路退回(平台判错了,平台自己认)'
        : '撤销骑手责任,判责时扣骑手的钱退回、保障金池出的回池(平台判错了,平台自己认)'
    case 'cancel_split':
      return '顾客在取消分摊里承担的部分,由平台原路退回(平台判错了,平台自己认)'
 ...
 * 动不同的东西(见 [overturnEffect]):骑手的钱退回、商家冲掉的钱补回、顾客的钱原路退 ——
 * 都由平台出(平台判错了,平台自己认),确认框里照实写。
 */

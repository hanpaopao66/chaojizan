 ...
 * 问题,判商家责任:顾客全额退款,商家这单净额冲回、骑手那份配送费和小费另出;其余(餐损、丢餐……)
 * 判骑手责任:顾客全额退款,这单骑手收入冲回,商家那份餐钱先由骑手保障金池出、不够的从骑手收入里扣
 * (两种都是平台不出钱)。
...
function actionsFor(issue: DeliveryIssue): { value: IssueAction; label: string; effect: string }[] {
  ...
  const charge = issue.merchant_charge_preview_cents ? yuan(issue.merchant_charge_preview_cents) : ''
  return [
    ...
    merchantFault
      ? { value: 'refund', label: '判为商家责任,商家承担全额退款',
          effect: `到店没出餐、餐没装齐,是商家那一环的问题:订单结束,全额${amount ? ` ${amount}` : ''}`
            + '(含配送费和小费)退给顾客,钱由商家出 —— 这单的净额冲回(平台佣金也不收),'
            + `骑手跑了这一趟,配送费和小费照拿,这一份${charge ? ` ${charge}` : ''}也由商家另出。`
            + '记为商家责任,扣商家信用分、不扣骑手的;商家 72 小时内可以在「售后判责」申诉' }
      : { value: 'refund', label: '判为骑手责任,全额退款',
          effect: `餐在配送途中损坏、丢失,责任在骑手:全额${amount ? ` ${amount}` : ''}退给顾客并结束订单。`
            + '商家无责、净额不动;这单骑手的配送费不计、平台这单佣金不收;商家那份餐钱先由骑手保障金池出,'
            + '池子不够的从骑手收入里扣(平台不出钱)。记为骑手责任,扣骑手信用分;'
            + '骑手 72 小时内可以申诉,成立的话扣的钱退回' },
  ]
}

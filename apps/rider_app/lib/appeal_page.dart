...
/// ## 界面上必须说清楚的一句
///
/// **申诉成立不加分也不补钱** —— 这里申诉的超时、差评本来就不进骑手信用分
/// (信用分只算平台判定成立的配送异常、售后判责和违规,见 server/app/services/credit.py),
/// 平台也没有服务分、派单分这类东西,所以没有分可加。申诉的价值是这条记录上写着不怪你。
/// 不说清楚的话骑手会以为申诉能拿到补偿,结果是更大的失望。
/// 判为骑手责任、会扣信用分的那几类,申诉入口在「我的信用分」里那一条旁边。
///
/// ## 证据不用骑手自己找
///
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '超时和差评本来就不扣信用分,平台也没有服务分、派单分,'
                            '所以**没有分可加、也不会补钱**。申诉的意义是记录上写清楚不怪你,'
                            '以及平台据此去看商家出餐那一环。'
                            '判为骑手责任的配送异常、售后和违规会扣信用分,'
                            '在「我的信用分」里那一条旁边申诉。\n'
                            '证据不用你自己找 —— 等餐时长、实际距离、'
                            '天气豁免提交时会自动附上。',
                            style: TextStyle(

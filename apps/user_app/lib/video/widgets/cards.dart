        ...
        if (showDuration)
          Positioned(
            ...
            child: Container(
              ...
              // 数字走等宽衬线:一排卡片的播放数、时长上下对得齐(设计稿 D)
              child: Row(children: [
                const Icon(Icons.play_circle_outline, size: 13, color: Colors.white),
                const SizedBox(width: 3),
                Text(vCount(card.views), style: szTabular(fontSize: kFontMicro, color: Colors.white)),
                const SizedBox(width: 8),
                const Icon(Icons.subtitles_outlined, size: 13, color: Colors.white),
                const SizedBox(width: 3),
                Text(vCount(card.danmakuCount), style: szTabular(fontSize: kFontMicro, color: Colors.white)),
                const Spacer(),
                Text(vDuration(card.durationMs), style: szTabular(fontSize: kFontMicro, color: Colors.white)),
              ]),
            ),
          ),
        if (card.collab)

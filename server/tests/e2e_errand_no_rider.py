...
然后一直躺在抢单池里等骑手。而 `_sweep_no_rider` 的取消分支里原来有一句:

    if from_status == OrderStatus.READY:   # 商家已出餐 → 平台赔餐损

(2026-09-15 起外卖也不赔了,这一支整个删掉;这条用例留着,守的是「取消单上不许冒出
商家入账」—— 哪天有人把赔付加回来,跑腿单不能跟着被赔。)

这句话当时对外卖的理由是 —— 商家真的把餐做出来了,运力不足是平台的问题。
但对跑腿是**凭空造钱**:那个 merchant_id 指向的是每城一个的

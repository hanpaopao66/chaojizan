...
- **非完成订单不许挂来路不明的入账行**:上面 1/6/7 全部从 completed 出发遍历,
  ...
  而无骑手兜底取消以前会往那儿写赔付行(auto_flow;2026-09-15 起不再写,历史行照认);
...
#: 无骑手兜底取消时给商家的餐损赔付行的 note 前缀。**2026-09-15 起不再产生**(无人接单取消
#: 不再赔餐损,auto_flow._sweep_no_rider),之前赔出去的行照旧留着 —— 这里判定"是历史合规赔付、
#: 不是野账",routers/transparency.py 的 /funds 按它公示历史赔付支出。两处共用这一个前缀。
...
def _rider_due(order) -> int:
    ...
    **不是**"顾客付的配送费" —— 两者在三种情况下不相等,
    ...
    - **等餐补偿**(历史):平台出的钱、不进 `delivery_fee_cents`,但确实进了骑手入账。
      2026-09-14 起停发,新单不会再有;之前已经结过的那些单照历史口径算(fee_parts.wait);
...
async def run_audit() -> list[dict]:
    ...
    async with SessionLocal() as db:
        ...
        # 谁都遍历不到。而无骑手兜底取消(services/auto_flow.py)以前会往那儿
        # 写一条平台赔付餐损的入账行 —— 那是真金白银从平台流向商家,
        # 却是审计视野里唯一一块没人看的地方。(2026-09-15 起不再写,历史行照认)
        #

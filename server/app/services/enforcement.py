...
拿分数来决定处置,分数就影响他能不能接单、能不能营业 —— 是绳索。
除此之外还有三件实际会发生的事:
...
顾客、商家、骑手现在都有一个信用分(services/credit.py)。它和处置是两回事,而且刻意分开:

- **处置不读信用分。** 限制、冻结只按下面的目录计次算(level_for),分数高低不改变任何人的
  处置级别。tests/unit/test_credit.py 守着:这个模块、派单、定价的代码里不许出现它;
- 信用分只是把「判定成立、能申诉」的记录摊成一个数,只在接单之后给这一单的交易对方看,
  不进派单、定价、排序、曝光、搜索。上面三条它逐条回应过(每一分指回一条记录、只扣判定
  成立的事、不影响谁能看到哪一单),见那个模块的抬头;
- 仍然成立的公开承诺:没有拿来处罚的「违规积分」;出餐时长、送达时长、评分这些
  「做得怎么样」的指标不折算成分数;骑手没有服务分、派单分、段位这类影响接单的东西。
...
#: 慢、少、晚都是能力和条件的问题,不在这里;慢和晚也不折算成任何分数。
CATALOG: tuple[Rule, ...] = (
    # ---- 用户 ----

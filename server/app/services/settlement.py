...
async def credit_rider_for_order(db: AsyncSession, order: Order) -> None:
    ...
    if existing:
        return
    # 等餐补偿(#145,平台出钱)2026-09-14 停发:平台没有钱,不做平台出钱的赔付。
    # 等餐时长照旧记录(到店时刻、取餐时刻都在订单上)、照旧公示,也不向商家收钱
    # (出餐时长君子协定)。之前已经结给骑手的补偿不动,审计照历史口径认(fee_parts.wait)

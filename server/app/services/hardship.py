...
2. **难度只会变准,不会被"摊平"。** 严禁"这个地址大家都说难,
   ...
   同一种自我收紧,换了个地方;
3. **不当场补钱,以后的单顾客付、下单前看得到。** 原来反馈的这一单当场补钱、平台出;
   2026-09-14 拍板「平台没有钱,不做平台出钱的安抚和营销」之后停了。反馈只用来给
   **以后**的单定价:攒够共识后,这个地址的新单在下单时就把难度费算进配送费(全归骑手),
   顾客下单前看得到、可以改选送到楼下省这笔钱。反馈的这一单也不向顾客或商家追收 ——
   顾客下单时没看到这笔钱,事后追收就是加价,骑手也会因此不敢说;
...
def comp_cents(kinds: list[str], floors: int | None,
    ...
    下单时拿地址的共识调它(算进配送费,顾客付、归骑手);骑手反馈时拿来给他看
    「以后这里的单会加多少」—— 2026-09-14 起反馈的这一单不再当场补钱(见模块抬头第 3 条)。

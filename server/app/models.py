...
class Order(Base):
    ...
    # 「等餐超 X 分钟扣商家分」—— 不做:等餐不进商家信用分(那里只算平台判定成立的事),
    # 也不进处置。
...
class StaffHealthCert(Base):
    ...
    所以只提醒 + 进合规档案 + admin 可见,不落闸,也不进商家信用分 ——
    证过期是一条提醒,不是平台判定成立的一件事。
    """
...
class RiderAppeal(Base):
    ...
    **只把这一单标注为「非骑手责任」,不加回任何分数** —— 超时、差评本来就
    不进骑手信用分(services/credit.py 只算平台判定成立的事),平台也没有服务分、
    派单分这类东西。所以申诉的价值是"这条记录上写着不怪我",不是换钱。
    界面上必须说清楚,否则骑手会以为申诉能拿到补偿。

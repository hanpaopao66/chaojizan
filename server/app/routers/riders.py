...
async def submit_appeal(
    ...
    **只把这一单标注为「非骑手责任」,不加回任何分数、不补偿金额** ——
    这里申诉的超时、差评本来就不进骑手信用分(services/credit.py 只算平台判定成立的
    配送异常、售后判责和违规),所以没有"分"可加。申诉的价值是这条记录上写着不怪你。
    不说清楚的话骑手会以为申诉能拿到钱。判为骑手责任的那几类有自己的申诉入口
    (原来的 appeals 通道,或者「我的信用分」里那一条旁边的工单申诉)。
    """
    from datetime import datetime, timezone
...
async def my_appeals(
    ...
    return {
        ...
        "note": "成立 = 这一单标注为非骑手责任。超时和差评本来就不扣信用分,"
                "所以没有分可加 —— 但记录上会写清楚不怪你。"
                "判为骑手责任的配送异常、售后和违规,在「我的信用分」里那一条旁边申诉。",
    }
...
async def mark_arrived_shop(
    ...
    **只记录,不判罚。** 有了它之后很容易顺手加一条「等餐超 X 分钟扣商家分」
    —— 不做:等餐是「做得怎么样」,不进商家信用分(那里只算平台判定成立的事),
    也不进处置。

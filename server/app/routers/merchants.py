...
async def my_compliance(
    ...
    **这一页的记录不折算成分数**。代码里三处写死的承诺是同一个立场:
    出餐时长「不用于排名、扣分、影响曝光」(见 /me/prep-time 的
    never_used_for)、/me/quality「只统计不处罚」、明厨亮灶「不做 AI 打分」。
    一旦拿这些做分数,它迟早会影响谁被看见,而误判会落到具体的人身上。

    商家信用分(services/credit.py,2026-09-14 拍板)是另一回事:它只算平台判定成立、
    能申诉的事(违规成立、平台复核判为商家责任的售后),只在接单之后给这一单的顾客、
    骑手看,不进排序、曝光、搜索和处置 —— 这一页上的食安投诉、被驳回的图、出餐超时都不进它。
    ...
    return {
        ...
        "never_used_for": "这一页的记录不折算成分数、不用于排名、不影响你在用户端的曝光。"
                          "商家信用分只算平台判定成立、能申诉的事(违规成立、平台复核判为"
                          "你的责任的售后),只在接单之后给这一单的顾客和骑手看,不进排序、"
                          "曝光和处置;没有拿来处罚的「违规积分」—— 处置照旧按违规计次。",
    }


# ---------- 规则中心(数字从代码里的真实常量算出来) ----------

def _credit_sections_for_merchant() -> list[dict]:
    """「信用分」「交易对方的信用分」两节 —— 和公开规则页(services/rules.py)、
    透明中心是同一份,从 services/credit 的常量生成。"""
    from ..services.credit import counterpart_lines, rules_lines
    return [{"title": "信用分", "items": rules_lines("merchant")},
        ...
        "sections": [
            ...
            _no_rider_rules_section(),
            {
                ...
                "items": [
                    ...
                    "售后判责改判成立:这单冲掉的净额和另出的配送费、小费都补回你的收入,"
                    "钱由平台出(平台判错了,平台自己认);顾客拿到的退款不追回",
                ],
            },
            {
                "title": "配送责任",
                "items": [
                    "平台骑手送的单,配送是骑手的事:评价里的配送标签(送得慢/餐洒了)"
                    "只算骑手的分,不影响你的店铺评分",
            ...
            },
            {
                "title": "不会发生的事",
                "items": [
                    # 商家也有信用分之后(2026-09-14),「平台不给商家打分」这句不再成立。
                    # 还成立的是下面这些 —— 公示的话要在坏情况下也是真的
                    "没有拿来处罚的「违规积分」:处置照旧按违规计次,信用分高低不影响限制、"
                    "冻结,也不影响排序和曝光;出餐时长、评分、退款率这些经营指标都不折算成分数",
                    "评价不能删也不能花钱删,唯一例外是申诉成立后隐藏(评分同步扣回)",
                    "自配送的单配送费归你;平台配送的配送费全归骑手,平台一分不抽",
                ],
            },
            # 从 services/credit 的常量生成,和公开规则页、透明中心是同一份
            *_credit_sections_for_merchant(),
        ],

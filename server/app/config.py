...
class Settings(BaseSettings):
    ...
    # 群消息推送按成员分批扫,一批多少人(chat_push.notify_message)。
    # 群上限是 20 万(D7),一次性把成员全查出来会把库打趴 —— 这个数只影响
    # 每批多大,不影响谁该收到。**e2e 把它设成 2**,让几个人的小群也横跨好几批,
    # 少扫一批立刻就露馅;不然分批这段代码在测试里永远只走一趟
    chat_push_chunk: int = 500
    ...
    # 取消线:顾客全额退款;商家已出餐的餐损不赔(2026-09-15 起,平台没有钱),
    # 商家端在出餐按钮上事前提醒「建议骑手接单后再出餐,或改为自己配送」
    ...
    # 信用分的扣分从哪一天起算(北京日期,含这一天;services/credit.py)。
    #
    # 这天之前的配送异常裁决、售后判责、违规判定一律不扣分:以前后台处理配送异常的那个按钮
    # 叫「退款」,判的人不知道自己是在判谁的责任、会扣谁的分 —— 拿那时候的裁决扣分,
    # 等于事后改了规则。完成订单的加分**不受它影响**,照常看最近 180 天:那是当事人实实在在
    # 做完的单,不存在「当时不知道会算分」的问题,算上只对他有利。
    # 公示在透明中心「信用分怎么算」(改这里公示跟着变);它早于时间窗起点之后自然不再起作用。
    # e2e 要验「过了 72 小时申诉期还计分」「起算日之前的不算」,CI 里设成 30 天前
    credit_count_from: date = date(2026, 9, 15)

    # 骑手保障金:每完成一单从平台佣金中计提(分),用于骑手意外险采购;判骑手责任(洒餐/丢餐)时
    # 先垫商家那份餐钱,池子不够的从骑手收入里扣(services/rider_fault)。逐日计提额

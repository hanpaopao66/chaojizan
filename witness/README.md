...
2. **分布式留存**:你的节点把见过的锚点存在你自己的机器上
   ...
   5 分钟内发现,并在 [平台节点页](https://chaojizan.cc/nodes) 公开示警。
3. **逐行核账**:节点重算每一行流水——佣金是否 ≤ 承诺上限(当前 5%,费率内嵌在每日账本里,历史锚点按当天口径复算)、净额恒等式、
   配送费是否只进不冲、团购服务费是否恰好等于承诺费率(当前 2%);2026-09 起还核:判责行的种类和正负号
   (商家责任另出的配送费和小费、骑手责任另出的那部分)、骑手保障金池只有正数的支出和回池、
   申诉改判平台补回的每一笔是正数、各项合计与逐行加总一致。四个实现(Python、Go、网页、App)跑同一份用例
   `witness/testdata/verify_rows_cases.json`。
...
[Windows](https://chaojizan.cc/appdist/witness/chaojizan-witness-windows.exe) ·
[macOS](https://chaojizan.cc/appdist/witness/chaojizan-witness-macos.zip) ·
[Ubuntu/Linux](https://chaojizan.cc/appdist/witness/chaojizan-witness-linux.tar.gz)
...
这里的 Python 版、Go 版都是**我们写的** —— 「社区见证」的说服力上限,
取决于一个陌生人能不能不读我们的代码、只读一份规格就写出自己的验证器。

规格在 [docs/LEDGER-SPEC.md](../docs/LEDGER-SPEC.md):锚点格式、
哈希的精确定义(包括最容易实现错的那个细节)、逐行核账算法、
三组测试向量(其中两组是生产链上的真实锚点)。对得上向量,
按规格文末的方式开个 issue,我们把你的实现列进节点页 ——
从那天起,这个目录里的代码就只是「参考实现」了。
